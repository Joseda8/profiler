...
def multiply_row_block(start_row_index: int, end_row_index: int, left_matrix: List[List[int]], right_matrix_transposed: List[List[int]]) -> None:
    """Multiply a block of rows from the left matrix by the right matrix (transposed)."""
    for row_index in range(start_row_index, end_row_index):
        ...
        for column_values in right_matrix_transposed:
            sum(left_value * right_value for left_value, right_value in zip(row_values, column_values))
    return
...
def run_matmul(left_matrix: List[List[int]], right_matrix_transposed: List[List[int]], num_workers: int) -> None:
    """Compute matrix multiplication using a thread pool."""
    matrix_size = len(left_matrix)
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_checksums = [
            executor.submit(multiply_row_block, start_row_index, end_row_index, left_matrix, right_matrix_transposed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--matrix_size", required=True, type=int, help="Square matrix size (size x size).")
    ...
    # Build matrices before profiling to keep measurement focused on multiplication
    left_matrix = build_matrix(matrix_size)
    right_matrix_transposed = transpose(build_matrix(matrix_size))
    time.sleep(3)

    set_tag("start_matmul")
    run_matmul(left_matrix=left_matrix, right_matrix_transposed=right_matrix_transposed, num_workers=num_workers)
