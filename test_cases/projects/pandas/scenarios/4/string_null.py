"""
This benchmark check if there are Null (None) values in a column
with type object using isnull().

Benchmark Steps:
1. Load user data into a DataFrame with a specified number of records.
2. Checks if there are Null values in the column.
...
are_there_none_values = df_users[name_column].isnull().any()
logger.debug(f"Does the column contain None values? - {are_there_none_values}")
