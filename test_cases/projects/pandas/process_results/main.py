...
class FileStats:
    ...
    def get_times(self) -> Dict[str, float]:
        ...
        for key in dict_labels_times:
            ...
            if key.startswith(LABEL_START_PREFIX):
                # Extract the task name from the key and create finish tag
                task_name = key[len(LABEL_START_PREFIX):]
                finish_key = LABEL_FINISH_PREFIX + task_name
    ...
    def get_average_stats(self) -> Optional[Tuple[float, float, float, float]]:
        """
        Compute the average CPU usage, virtual memory usage, RAM usage, and swap usage of the program
        between specific labels.

        Returns:
            Optional[Tuple[float, float, float, float]]: A tuple containing the average CPU usage,
            average virtual memory usage, average RAM usage, and average swap usage of the program between specific labels.
            Returns None if either label is not found.
        """
        # Calculate program average stats
        average_stats = self.get_average_between_labels(start_label=LABEL_START_PROGRAM, finish_label=LABEL_FINISH_PROGRAM)
    ...
    def get_min_max_stats(self) -> Optional[Tuple[float, float, float, float, float, float]]:
        """
        Get the minimum and maximum memory stats of the program between specific labels.

        Returns:
            Optional[Tuple[float, float, float, float, float, float]]: A tuple containing the minimum and maximum values
            of virtual memory usage, RAM usage, and swap usage of the program between specific labels.
            Returns None if either label is not found.
        """
        # Calculate minimum and maximum memory stats between specific labels
        min_max_stats = self.get_min_max_memory_stats(start_label=LABEL_START_PROGRAM, finish_label=LABEL_FINISH_PROGRAM)
...
print(file_stats.get_times())
print(file_stats.get_average_stats())
print(file_stats.get_min_max_stats())
